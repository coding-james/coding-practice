    ...
    public class battleshipsValid
    {
        public static bool ValidateBattlefield(int[,] field)
        {
            ...
            for (int y = 0; y < 10; y++)
            {
                for (int x = 0; x < 10; x++)
                {
                    Tuple<int[], int> temp = shipFinder(shipFound, field, direction, len, y, x, x, x, y, y + 1, y - 1); // across x are all equal. y1 and y2 varies +1 or -1
                ...
                if (len > 0)
                {
                    shipFound = shipCheck(len, shipFound, direction);
                    len = 0;
                }
            }
            ...
            for (int x = 0; x < 10; x++)
            {
                for (int y = 0; y < 10; y++)
                {
                    Tuple<int[], int> temp = shipFinder(shipFound, field, direction, len, x, x, x + 1, x - 1, y, y, y); // across y are all equal. x1 and x2 varies +1 or -1
                    shipFound = temp.Item1;
                    len = temp.Item2;
                }
            ...
            for (int y = 0; y < 9; y++)
            {
                for (int x = 0; x < 9; x++)
                {
                    if (!diagonalsCheck(field, x, y, x + 1, y + 1))
                    {
                        return false;
                    }
                }
        ...
        private static Tuple<int[], int> shipFinder(int[] shipFound, int[,] field, string direction, int len, int xOrY, int x, int x1, int x2, int y, int y1, int y2) // Tuple allows multiple fields back
        {
        ...
        private static int[] shipCheck(int len, int[] shipFound, string direction)
        {
        ...
        private static bool diagonalsCheck(int[,] field, int x0, int y0, int x1, int y1)
        {
