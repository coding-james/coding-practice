...
const { zero, one, two, three, four, five, six, seven, eight, nine, plus, minus, times, dividedBy } = require("../5 kyu/calculating_with_functions");
...
describe("Calculating with Functions", () => {
    ...
    tests.forEach(({ arg, expected }) => {
        it(`should return ${expected}`, () => {
            assert.strictEqual(arg[0](arg[1](arg[2]())), expected);
        });
    });
});